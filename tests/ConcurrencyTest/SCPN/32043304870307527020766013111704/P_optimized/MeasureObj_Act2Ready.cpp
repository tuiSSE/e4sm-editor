/************************************************************
	...
	MeasureObj_Act2Ready.cpp  -  generated by CodeGenerator r$Rev$
...
MeasureObj_Act2Ready* measure_Act2Ready;

MeasureObj_Act2Ready::MeasureObj_Act2Ready(NetClass* net) : MeasureObj("Act2Ready", net, true)
{
	firstTime = lastTime = 0;
	value = 0;
}

double MeasureObj_Act2Ready::measure(double dt=-1)
{
	return value;
}

void MeasureObj_Act2Ready::accum(const Binding *binding, double weight) {
	const DateTime &t = binding->getTime();
	double dt = t.FullDateTime() - lastTime;
	lastTime = t.FullDateTime();

	// Orig Expression: #Act2_executed
	// C++  Expression: #P000031
	Place *P000031 = net->getPlace("Act2_executed");

	const Transition *trans = binding->getBoundTransition();
	const TokenList *tokens = binding->getBoundTokens();

	double rate = (P000031->getMarkingSize());

	value += weight * dt * rate;
}

MeasureObj_Act2Ready::~MeasureObj_Act2Ready()
{
}
